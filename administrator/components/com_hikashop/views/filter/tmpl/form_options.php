...
?>				<?php
				...
				<table class="paramlist admintable table">
					...
					<tr id="sort_by" <?php echo $sortDisplay; ?>>
						...
						<td>
							<?php
								echo $this->product_informationType->display("data[filter][sort_by][]", @$this->element->filter_options['sort_by'], $this->fields, 'class="custom-select" multiple="multiple" size="8"', null, true);
							?>
						</td>
					<tr/>
					...
					<tr id="cursorNumber" <?php echo $displayCursors; ?>>
						...
						<td>
							<select name="data[filter][cursor_number]" class="custom-select">
								<?php
								$maxColumn=5;
								for($i=1;$i<$maxColumn;$i++){  ?>
