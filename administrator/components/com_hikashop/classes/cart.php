...
class hikashopCartClass extends hikashopClass {
	...
	protected function checkQuantities(&$products, &$cart, $options = array()) {
		...
		if(!$wishlist && hikashop_level(1) && !empty($b_ids)) {
			$query = 'SELECT pr.product_id, MIN(FLOOR(p.product_quantity / pr.product_related_quantity)) as bundle_quantity '.
			...
			$this->db->setQuery($query);
			$bundles = $this->db->loadObjectList('product_id');
			if(!empty($bundles)) {
				foreach($bundles as $k => $bundle) {
					$db_products[ $id ]->product_quantity = (int)$bundle->bundle_quantity;
				}
			}
		}
		unset($b_ids);
		...
		if(!$wishlist && hikashop_level(1)) {
			...
			if(!empty($b_ids)) {
				...
				foreach($cart->products as $cart_product_id => &$product) {
					if(empty($product->product_id))
						continue;
					if(!isset($bundles[(int)$product->product_id]))
						continue;
					$product->bundle_quantity = (int)$bundles[(int)$product->product_id]->bundle_quantity;
				}
				unset($product);
			}
			unset($b_ids);
